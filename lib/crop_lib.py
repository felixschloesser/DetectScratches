...
if __name__ == '__main__':
    from image_lib import resize_img, load_img, show_img

    img = load_img("c02.jpg", size=(600, 800))
    print(img.shape)
    height, width = img.shape
    border, scale = mark_region(img)

    line_points = fit_lines(border, scale)

    print(border.shape)
    border, _ = resize_img(border, img.shape)
    print(border.shape)

    fig, ax = plt.subplots()
    ax.imshow(img+ border*100, cmap="gray")
    ...
    intersections = find_intersections(line_points, img.shape)
    warped = warp_to_corners(img, *intersections)
    show_img(warped, size=(10,10))

else:
    from lib.image_lib import resize_img
